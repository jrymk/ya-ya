...
struct coord {
    ...
    double y = 0.;
    inline coord() {};
    inline coord(double x, double y) {this->x = x; this->y = y;}
    inline coord(sf::Vector2f v2) {this->x = v2.x; this->y = v2.y;}
...
class Camera {
    ...
    inline static double zoom = 200.; // the span of the camera, in sqrt(w*h) coord space
...
public:
    ...
    inline static void setZoom(double z) { zoom = z; updateScale(); }  
    inline static double getZoom() { return zoom; }
    inline static double getScale() { updateScale(); return scale; }
